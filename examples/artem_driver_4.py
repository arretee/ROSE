...
def getBestWay(world, player_x, player_y, score, min_x, max_x, until_finish, cells_to_ignore):
    ...
    if [player_x, player_y - 1] in cells_to_ignore:
        ...
        for act in acts:
            if min_x <= player_x + act[0] <= max_x:
                ...
                if world.get(pos) == obstacles.NONE or world.get(pos) == obstacles.PENGUIN:
                    temp_score += 10
                    temp_score = getBestWay(world, player_x, player_y, temp_score, min_x, max_x, until_finish - 1, cells_to_ignore)[1]
                ...
                elif pos not in cells_to_ignore:
                    temp_score -= 10
                    cells_to_ignore.append(pos)
                    temp_score = getBestWay(world, player_x, player_y + 1, temp_score, min_x, max_x, until_finish - 1,
                                            cells_to_ignore)[1]
                    cells_to_ignore.remove(pos)
        ...
        for act in [NONE, LEFT, RIGHT]:
            if min_x <= player_x + act[0] <= max_x:
                ...
                if world.get(pos) == obstacles.NONE or world.get(pos) == obstacles.PENGUIN:
                    ...
                    temp_score -= 10
                    cells_to_ignore.append(pos)
                    temp_score = getBestWay(world, player_x, player_y + 1, temp_score, min_x, max_x, until_finish - 1, cells_to_ignore)[1]
                    cells_to_ignore.remove(pos)
